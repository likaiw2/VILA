...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--question-file", type=str, default=".data/ReVA_V1/final_valid_set.json")
...
def resolve_video_path(raw_path: str, dataset_root: str, dataset_prefix: str) -> str:
    ...
    if raw_path.startswith(dataset_prefix):
        ...
        candidates.append(Path(dataset_root) / suffix_path)

        parts = suffix_path.parts
        if len(parts) > 1:
            # Some annotation files use a logical dataset namespace like
            # `#dataset/RSVidQA/...`, while the local tree is rooted at
            # `.data/ReVA_V1/...`. In that case, dropping the first segment
            # resolves the actual media path.
            candidates.append(Path(dataset_root) / Path(*parts[1:]))
    ...
    raise FileNotFoundError(
        f"Cannot resolve video path '{raw_path}'. "
        f"Tried: {', '.join(seen)}"
    )
